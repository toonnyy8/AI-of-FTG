...
export class DDDQN {
    ...
    buildModel(
        ...
        outputLayer = tf.layers.dense({ units: stateVectorLen, activation: "selu" }).apply(outputLayer)
        outputLayer = tf.layers.dense({ units: stateVectorLen, activation: "selu" }).apply(outputLayer)
        outputLayer = tf.layers.dense({ units: actionsNum.reduce((prev, curr) => prev + curr, 0), activation: "selu" }).apply(outputLayer)
