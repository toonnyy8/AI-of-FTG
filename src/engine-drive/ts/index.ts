                ...
                load.onchange = (event) => {
                    ...
                    reader.addEventListener("loadend", () => {
                        myz.load(<ArrayBuffer>reader.result).then((datas) => {
                            ...
                            ctrl1 = Array.from(<Uint8Array>datas.find((data) => {
                                return data.name == "ctrl1"
                            }).values)
                            ctrl2 = Array.from(<Uint8Array>datas.find((data) => {
        ...
        ; (<HTMLButtonElement>document.getElementById("train-log")).onclick = async () => {
            ...
            let train = () => {
                    ...
                    tf.tidy(() => {
                        ...
                        op.minimize(
                            () => {
                                ...
                                let out_dec = dec_fn(<tf.Tensor4D>out_driver)

                                // return tf.sub(1, nn.ssim2d(y, out).mean())
                                return tf.losses.logLoss(y_dec, out_dec).add(tf.losses.meanSquaredError(y_enc, out_driver))
                            },
                            true,
                ...
                tf.tidy(() => {
                    ...
                    let test_yImg = <tf.Tensor3D>dec_fn(enc_fn(trainData.slice([batchStart + batchSize + (t - 1), 0, 0, 0], [1, -1, -1, -1]))).squeeze([0])
                    let ctrl1Batch = ctrl1.slice(batchStart, batchStart + batchSize)
                    let ctrl2Batch = ctrl2.slice(batchStart, batchStart + batchSize)
                    let test_out = <tf.Tensor3D>tf.tidy(() => (<tf.Tensor>dec_fn(driver.fn(<tf.Tensor4D>enc_fn(test_xImg), [ctrl1Batch, ctrl2Batch]))).slice([batchSize - 1, 0, 0, 0], [1, -1, -1, -1]).squeeze([0]))
