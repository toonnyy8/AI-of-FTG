            ...
            for (let j = 0; j < loop; j++) {
                ...
                tf.tidy(() => {
                    ...
                    let x_z = tf.tidy(() => {
                        ...
                        return mu //reparametrize(mu, logvar)
                    })
                    let y_mus = yImgs.map((yImg) =>
                        tf.tidy(() => {
                            const { mu } = enc_fn(yImg)
                            return mu
                        })
                    )
                    let mask = (() => {
                    ...
                    const { gradients, loss } = y_mus.reduce(
                        (prev, y_mu, idx) => {
                        ...
                        { gradients: <tf.NamedTensorMap>{}, enc: x_z.clone(), loss: tf.scalar(0) }
                    )
                    loss.div(t).print()
    ...
    ;(<HTMLButtonElement>document.getElementById("test")).onclick = () => {
        ...
        if (runTest) runTest = false
            ...
            let input_enc = tf.tidy(() => {
                const { mu } = enc_fn(trainDatas[fileIdx].slice([0, 0], [L, -1]))
                return mu
            })
