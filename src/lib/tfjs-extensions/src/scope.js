...
export function registerScope(tf_ = tf) {
    class VariableScope {
        ...
        getVariable(name, shape, dtype = "float32", initializer = tf_.initializers.randomNormal({ mean: 0, stddev: 1, seed: 1 }), trainable = true) {
            dtype = dtype != null && dtype != undefined ? dtype : "float32"
            initializer = initializer != null && initializer != undefined ? initializer : tf_.initializers.randomNormal({ mean: 0, stddev: 1, seed: 1 })
            ...
            if (Object.keys(this._variables).find((variablesName) => name == variablesName) == undefined) {
                this._variables[name] = tf_.tidy(() => {
                    return tf_.variable(initializer.apply(shape, dtype), trainable, `${this.scopeName}/${name}`, dtype)
                })
            }
